...
A name assigned by \=: will be accessible from other verbs. This contrasts with [Is (Local)](https://code.jsoftware.com/wiki/Vocabulary/eqdot "Vocabulary/eqdot") (\=.) which assigns to names that are visible only in the verb where they are defined.

**Note:** the value assigned need not be a noun. In the example below, the execution of 3 : 0 produces a verb, which is then assigned to the name verb1. After that, verb1 behaves as a verb.
...
\_5
   name\_loc\_ =: 2 3 4  NB. assign in locale loc
